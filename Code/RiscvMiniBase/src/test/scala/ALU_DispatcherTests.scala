...
class ALU_DispatcherTester(c: => ALU_Dispatcher)(implicit p: freechips.rocketchip.config.Parameters) extends BasicTester with TestUtils {
  ...
  val from_alu1        = Seq.fill(size)(rnd.nextBoolean())
  val from_alu2        = Seq.fill(size)(rnd.nextBoolean())
  val out              = (rs1 zip rs2 zip alu_op zip from_alu1 zip from_alu2) map { case ((((a, b), c), d), e) => dispatcher_model.step(a, b, c, d, e)}
  val a_toALU1         = VecInit(out map { case Array(a, b, c, d, e, f) => a.U})
  val b_toALU1         = VecInit(out map { case Array(a, b, c, d, e, f) => b.U})
  val alu_op_toALU1    = VecInit(out map { case Array(a, b, c, d, e, f) => c.U})
  val a_toALU2         = VecInit(out map { case Array(a, b, c, d, e, f) => d.U})
  val b_toALU2         = VecInit(out map { case Array(a, b, c, d, e, f) => e.U})
  val alu_op_toALU2    = VecInit(out map { case Array(a, b, c, d, e, f) => f.U})
  ...
  printf("Counter: %d, OP: %d, A: %d, B: %d, from_alu1: %d, from_alu2: %d, OUT: (hw) %d %d %d %d %d %d ?= (sw) %d %d %d %d %d %d\n",
         cntr, dut.io.alu_op, dut.io.A, dut.io.B, dut.io.from_alu1, dut.io.from_alu2, dut.io.A_toALU1, dut.io.B_toALU1, 
         dut.io.alu_op_toALU1, dut.io.A_toALU2, dut.io.B_toALU2, dut.io.alu_op_toALU2, a_toALU1(cntr), b_toALU1(cntr),
         alu_op_toALU1(cntr), a_toALU2(cntr), b_toALU2(cntr), alu_op_toALU2(cntr)) 
}
